...
sum_up.scans<- function(scan_list,scaled=FALSE,
                        method = c("theoretical","group","focal","both"),mode = c("directed", "undirected", "max","min", "upper", "lower", "plus","vector")){
  method<- match.arg(method);
  switch(method,
         "theoretical" = {
           list(
             theoretical = sum_up.scan(scan_list,"theoretical",scaled = scaled),
           )
         },
         "group" = {
           list(
             theoretical = sum_up.scan(scan_list,"theoretical",scaled = scaled),
             group = sum_up.scan(scan_list,"group",mode = mode,scaled = scaled)
           )
         },
         "focal" = {
           list(
             theoretical = sum_up.scan(scan_list,"theoretical",scaled = scaled),
             focal = sum_up.scan(scan_list,"focal",mode = mode,scaled = scaled)
           )
         },
         "both" = {
           list(
...
#' @examples
#' #Internal use for readability
sum_up.scan<- function(scan_list,method,mode=NULL,scaled){
  scan_list.method<- lapply(scan_list,function(l) l[[method]]);
  ...
  if(scaled){
    summed_up.method<- summed_up.method/n.observed_edges(scan_list.method,diag = 1) # here and hereafter diag = 1 because while the count of the diagonal is irrelevant, it shouldn't be 0/0.
