...
#'
#' @examples
simulate.zeroes.non.zeroes<- function(total_scan,prob){
  scan_list<- vector(mode="list",length = total_scan)
  non.zeroes<- rbinom(total_scan,1,1-prod(1-prob$present))==1;
...
#'
#' @examples
adjust.conditional.prob<- function(prob){
  prob.all.zeroes<- 1-prod(1-prob$present)
  previous.are.zeros<- c(1,cumprod(1-prob$present[1:(n-1)]))
  sapply(1:n,function(i) prob$present[i]/prob.all.zeroes*previous.are.zeros[i])
}
...
#'
#' @examples
do.non.zero.scan<-function(scan_list,prob){
  n<- length(prob$present)
  Scan<- matrix(data = 0,nrow = n,ncol = n,dimnames = list(nodes_names,nodes_names))

  scan<- rep(0,n)
  rand.order<- sample(1:n,n)
  P.cond<- cumsum(adjust.conditional.prob(prob$present[rand.order]))
  first.one<- min(which(runif(1)<P.cond))
  scan[rand.order][first.one]<- 1
  scan[rand.order][-first.one]<- rbinom(n-1,1,P[rand.order][-first.one])
  scan
}
